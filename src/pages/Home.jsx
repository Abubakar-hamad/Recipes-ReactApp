              ...
              recipes.map(recip =>{
                  return(
                    <SplideSlide onClick={()=> navigate(`/cuisine/${recip.id}/info`) }   key={recip.id}  className=' relative  cursor-pointer h-56 '>
                      ...
                      <p className='absolute overflow-hidden md:text-xl sm:text-sm text-center justify-center hover:scale-105  transition-all items-center inset-x-10 h-fit bottom-8 bg-gray-300 bg-opacity-75'>{recip.title}</p>
                    </SplideSlide >
                  )
              })
              :
              ...
              veggi.map(veg =>{
                  return(
                    <SplideSlide onClick={()=> navigate(`/cuisine/${veg.id}/info`) }   key={veg.id}  className=' cursor-pointer  h-56'>
                     ...
                     <p className='absolute overflow-hidden md:text-xl sm:text-sm text-center justify-center items-center inset-x-10 h-fit bottom-8 hover:scale-105  transition-all bg-gray-300 bg-opacity-75'>{veg.title}</p>
                    </SplideSlide >
                  )
              })
              :
              ...
              dessert.map(dess =>{
                  return(
                    <SplideSlide onClick={()=> navigate(`/cuisine/${dess.id}/info`) } key={dess.id}  className=' cursor-pointer  h-56'>
                     ...
                     <p className='absolute overflow-hidden md:text-xl sm:text-sm text-center justify-center items-center inset-x-10 h-fit bottom-8 hover:scale-105  transition-all bg-gray-300 bg-opacity-75'>{dess.title}</p>
                    </SplideSlide >
                  )
              })
              :
